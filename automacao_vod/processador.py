...
GENRE_URL = "https://api.themoviedb.org/3/genre/{type}/list"
...
# Dicionário para cache de gêneros
GENRES_CACHE = {"movie": {}, "tv": {}}

def carregar_generos():
    """Busca a lista de gêneros oficial do TMDb para Filmes e Séries."""
    for tipo in ["movie", "tv"]:
        try:
            url = GENRE_URL.format(type=tipo)
            params = {"api_key": TMDB_API_KEY, "language": "pt-BR"}
            response = session.get(url, params=params, timeout=10)
            if response.status_code == 200:
                genres_list = response.json().get("genres", [])
                GENRES_CACHE[tipo] = {g["id"]: g["name"] for g in genres_list}
        except Exception as e:
            logger.warning(f"Erro ao carregar gêneros para {tipo}: {e}")

def buscar_metadados(nome_limpo: str) -> Tuple[str, str, str, str, str]:
    ...
    try:
        ...
        if data.get('results'):
            res = data['results'][0]
            media_type = res.get('media_type', 'movie') # movie ou tv
            ...
            tipo_chave = "tv" if media_type == "tv" else "movie"
            nomes_generos = [GENRES_CACHE[tipo_chave].get(gid) for gid in genre_ids if GENRES_CACHE[tipo_chave].get(gid)]
            genero_principal = nomes_generos[0] if nomes_generos else ""
            
            return poster, ano, sinopse, tmdb_id, genero_principal
    except Exception as e:
...
def gerar_m3u():
    carregar_generos() # Inicializa os gêneros antes de processar
    pasta = "automacao_vod"
    arquivos_config = {
            ...
            for nome_arq, info in arquivos_config.items():
                ...
                with open(caminho, "r", encoding="utf-8") as f_in:
                    ...
                    for i in range(0, len(linhas), 2):
                        ...
                        logo, ano, sinopse, tmdb_id, genero = "", "", "", "", ""
                        
                        if info["type"] in ["movie", "series"]:
                            logo, ano, sinopse, tmdb_id, genero = buscar_metadados(nome_original)
                        
                        # LOGICA DE GRUPO: Prioriza Gênero do TMDb > Grupo do arquivo original > Grupo padrão
                        grupo_original = extrair_tag(header, "group-title")
                        grupo_final = genero if genero else (grupo_original if grupo_original else info["default_group"])
                        ...
                        if final_tvg_id: tags.append(f'tvg-id="{final_tvg_id}"')
